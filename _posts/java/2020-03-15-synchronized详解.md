...
​		线程的合理使用能够提升程序的处理性能，主要有两个方面，第一个是能够利用多核 cpu 以及超线程技术来实现线程的并行执行；第二个是线程的异步化执行相比于同步执行来说，异步执行能够很好的优化程序的处理性能提升并发吞吐量同时，也带来了很多麻烦，如：**多线程对于共享变量访问带来的安全性问题**。而我们熟知的多线程并发编程中 *synchronized* 一直是元老级角色，很多人都会称呼它为重量级锁。但是，随着 Java SE 1.6 对*synchronized* 进行了各种优化之后，有些情况下它就并不那么重，Java SE 1.6 中为了减少获得锁和释放锁带来的性能消耗而引入的偏向锁和轻量级锁。
